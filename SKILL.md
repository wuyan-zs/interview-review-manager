...
1. Work on the named file or bounded directory only. A full-vault baseline requires an explicit request. Read the current filesystem path; do not substitute a backup, deleted sibling, or cached prior result unless the user explicitly asks for it.
...
8. Apply the evidence-first scoring fallback in [references/review-system.md](references/review-system.md#evidence-first-scoring-fallback). Preserve an explicit score or label exactly; never convert prose such as “答不上来” or “回答单薄” into a numeric score. When no comparable score exists, record the raw evidence (or `未记录`), use `首答：未量化`/`评分尺度：未知` as appropriate, and do not calculate a review date.
9. Distinguish `linked`, `source-only`, `partial`, `ambiguous`, `missing`, and `unlinked`.
10. If a named file contains more than 12 question units, show an inventory and process it in resumable batches of about 6–8 units unless the user explicitly requests one uninterrupted pass. Finish and validate each batch before starting the next; do not silently scan the whole vault.
11. For `partial`, `missing`, or `ambiguous` answer content, default to proposal-only: explain the exact gap, suggested destination, and smallest draft or patch, but do not create or edit a canonical knowledge/project answer until the user explicitly confirms. A request to organize or import permits source annotations and review-center markers, not unapproved canonical content. Project facts, responsibilities, metrics, and incidents require the user's confirmation.
12. If the user asked to update the review center, merge each weakness into one stable topic item with exact source-question and answer links. Do not copy whole answers into the center. A missing score alone is not a weakness; add a `待评估` item only when the source has an explicit gap or the user asks for a diagnostic.
13. Add or merge indexing metadata only for notes actually classified or link-enriched. Validate every link and anchor.
...
- For a topic with no comparable score, schedule a clearly labelled `闭卷诊断` (not a review or an overdue item); the pass criterion must produce the first evidence needed for later scoring. Do not invent a score or a due date.
...
4. Record the *first-answer* score, scale, and an evidenced session date only when the source supplies them. If the source has no score, preserve qualitative feedback or mark `首答：未量化`/`面试证据：未记录`; do not estimate, normalize, or treat an immediate retry as mastery.
...
- Scores are evidence, not a required field. Never fabricate, normalize across unknown scales, or infer a number from note quality. Conflicting score/label evidence pauses the write and is reported for confirmation.
