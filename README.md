...
评分不是必填项。Skill 会先保留原始证据，再决定是否进入复习队列：

- 有明确数字：原样保留评分和评分制；只有明确为 `0–10` 才使用 0–10 的间隔规则。
- 只有“通过 / 需加强 / 答不上来”等文字：保留原话或忠实摘要，不换算成 `3/10` 之类的数字。明确空白或关键错误可按证据给 `P0/P1`；没有可比评分时日期写 `待安排`。
- 完全没有作答反馈：不视为失败，记录 `首答：未量化`、`面试证据：未记录`，状态为 `待评估 · new`。只有用户要求建立基线时，才生成一条“闭卷诊断”任务。
- 评分、标签和文字互相矛盾：暂停自动更新，展示冲突原文，等你确认。

因此，复习中心里的 `待评估` 是“还没有证据”，不是“考了零分”；第一次闭卷诊断完成后，才开始计算后续复习日期。

## 和 DSH Interview Simulator 联动

两者分工明确：Interview Simulator 负责一问一答、追问、评分与反馈（如果它没有评分，也保留定性反馈）；本 Skill 负责保存结果、Obsidian 链接、识别答案缺口，以及复习计划。
...
复习日期只从有证据的作答日期和可比评分计算。没有明确日期或没有评分时，Skill 会写 `待安排`/`待评估`，不会把整理当天伪装成作答日期，也不会把定性文字伪装成数字。
